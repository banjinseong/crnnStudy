...
def run():
    ...
    os.makedirs(f'{save_path}/newElc8Label', exist_ok=True)
    os.makedirs(f'{save_path}/oldElc8Label', exist_ok=True)
    os.makedirs(f'{save_path}/oldElc7Label', exist_ok=True)
    ...
    for i in tqdm(range(count), desc='8-digit license plate(holographic)'):
        ...
        image_file_path = f'{save_path}/newElc8/{full_name}.png'
        image_pil.save(image_file_path, 'PNG')

        with open(f'{save_path}/newElc8Label/{full_name}.txt', 'w') as f:
            f.write(full_name)
    ...
    for i in tqdm(range(count), desc='8-digit license plate'):
        ...
        image_file_path = f'{save_path}/oldElc8/{full_name}.png'
        image_pil.save(image_file_path, 'PNG')

        with open(f'{save_path}/oldElc8Label/{full_name}.txt', 'w') as f:
            f.write(full_name)
    ...
    for i in tqdm(range(count), desc='7-digit license plate'):
        ...
        image_file_path = f'{save_path}/oldElc7/{full_name}.png'
        image_pil.save(image_file_path, 'PNG')

        with open(f'{save_path}/oldElc7Label/{full_name}.txt', 'w') as f:
            f.write(full_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--count', type=int, default=60, help='Number of image to save')
